...
#if defined(NDEBUG)
# error "Moduabit cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xaee9f1aae8bedb85eaad469dd98f98600bf6061e7999fad2e3bef8a6c04d12cc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Moduabit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Moduabit Signed Message:\n";
...
{
    // Moduabit: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (nHeight == 1 ) {
        int64 nSubsidy = 3000000000 * COIN;
        ...
        nSubsidy >>= (nHeight / 2628000); // Moduabit: 840k blocks in ~4 years
        return nSubsidy + nFees;
...
static const int64 nTargetTimespan = 1 *  60 * 60; // Moduabit: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Moduabit: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Moduabit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Moduabit: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x47f11bddf8473bee4a209478eb2e83a28e3e2281c1af27c0ce6f1e0102f1c3a6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Moduabit coin election birthday";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047ba8bab7e6d4db5774848dc638678fe614193b0be8cccbb2d5d1055da3525cac0d0001041dbdc15e24d594f7c6fa6fad34435496148e8c9c0c65c03d31e51566") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615114211;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084991288;

        if (fTestNet)
        {
            block.nTime    = 1615114160;
            block.nNonce   = 387451273;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2df65fb1312289d75309970848818759bdb599178c8bc2402e4a06596287e340"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xcb, 0xb3, 0xdc }; // Moduabit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ModuabitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ModuabitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ModuabitMiner(CWallet *pwallet)
{
    printf("ModuabitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("moduabit-miner");
    ...
    try { loop {
        ...
        printf("Running ModuabitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ModuabitMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ModuabitMiner, pwallet));
}
